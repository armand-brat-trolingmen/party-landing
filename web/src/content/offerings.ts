...
export type ServicePageTariffSection = {
  title: string;
...
export type ServicePageTariff = {
  ...
  items?: string[];
  variants?: ServicePageTariffVariant[];
  sections?: ServicePageTariffSection[];
};
...
const rawExtras: readonly OfferingEntity[] = [
  ...
  },
  {
    ...
    fullDescription:
      'Аренда оборудования подходит, когда для мероприятия нужна дополнительная техническая база или отдельные элементы для рабочей зоны. Мы уточняем задачу, площадку и формат, а затем подбираем оборудование под сценарий события. За оборудование берется залог, а его сумма согласовывается при формировании заказа и зависит от состава и количества позиций.',
    priceFrom: 'от 3 500 ₽',
    included: [
      ...
      'Аренда настольного морозильного ларя для шарикового мороженого — 6.000 ₽',
    ],
